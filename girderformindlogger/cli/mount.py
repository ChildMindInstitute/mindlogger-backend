...
from girderformindlogger import events, logger, logprint
...
from girderformindlogger.utility.server import configureServer
...
class ServerFuse(fuse.Operations):
    ...
    def _stat(self, doc, model):
        ...
        :param doc: the girderformindlogger resource document.
        :param model: the girderformindlogger model.
...
# You can add girderformindlogger to the list of known filesystem types in linux.  For
# instance, if you create an executable file at /sbin/mount.girderformindlogger that
# contains
#   #!/usr/bin/env bash
#   sudo -u <user that runs girderformindlogger> girderformindlogger mount -d "$@"
# then the command
#   mount -t girderformindlogger <database uri> <path> -o <options>
# will work.  If you specify a string that doesn't contain :// as the database
# uri, then it will use the default girderformindlogger configuration (e.g., use
# "mount -t girderformindlogger girderformindlogger <path>").
# If you have girderformindlogger installed in a virtualenv, ifrequently prepending the
# virtualenv's bin directory to the path is enough to use it, so the
# mount.girderformindlogger file becomes
#   #!/usr/bin/env bash
#   sudo -u <user that runs girderformindlogger> bash -c 'PATH="<virtualenv
#       path>:$PATH" ${0} ${1+"$@"}' girderformindlogger mount -d "$@"
