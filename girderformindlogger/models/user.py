...
class User(AESEncryption):
    ...
    def validate(self, doc):
        ...
        doc["displayName"] = doc.get("displayName", doc.get("firstName", "")).strip()
        ...
        if not doc["displayName"]:
            raise ValidationException("Display name must not be empty.", "displayName")
    ...
    def authenticate(
        ...
        if not self.hasPassword(user):
            e = events.trigger(
                "no_password_login_attempt", {"user": user, "password": password}
            )
    ...
    def remove(self, user, progress=None, **kwargs):
        ...
        Group().update({"requests": user["_id"]}, {"$pull": {"requests": user["_id"]}})
    ...
    def createUser(
        ...
        ) or self.findOne({"email": self.hash(email), "email_encrypted": True}):
            raise ValidationException(
                "That email is already registered in the system.",
            )
    ...
    def createTemplatesFolder(self, user):
        ...
        return Folder().setMetadata(templatesFolder, {"contentType": "templates"})
    ...
    def _sendVerificationEmail(self, user, email):
        ...
        token = Token().createToken(user, days=1, scope=TokenScope.EMAIL_VERIFICATION)
        url = "%s#useraccount/%s/verification/%s" % (
            mail_utils.getEmailUrlPrefix(),
            str(user["_id"]),
            str(token["_id"]),
        )
        text = mail_utils.renderTemplate("emailVerification.mako", {"url": url})
    ...
    def fileList(
        self, doc, user=None, path="", includeMetadata=False, subpath=True, data=True
    ):
        """
    ...
    def updateSize(self, doc):
        ...
        fixes = 0
        folderModel = Folder()
        folders = folderModel.find({"parentId": doc["_id"], "parentCollection": "user"})
        for folder in folders:
