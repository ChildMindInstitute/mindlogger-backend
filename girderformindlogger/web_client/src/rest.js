    ...
    const defaults = {
        ...
        error: (error, status) => {
            ...
            if (error.status === 401) {
                ...
                error.responseJSON.type === 'girderformindlogger') {
                info = {
 ...
 *                  xhr.girderformindlogger[category] set to a truthy value.
 * :returns: the number of outstanding requests.
 */
function numberOutstandingRestRequests(category) {
    if (category) {
        return _.filter(restXhrPool, function (xhr) {
            return xhr.girderformindlogger && xhr.girderformindlogger[category];
        }).length;
    }
...
function cancelRestRequests(category) {
    _.each(restXhrPool, function (xhr) {
        if (category && (!xhr.girderformindlogger || !xhr.girderformindlogger[category])) {
            return;
        }
        if (xhr.abort) {
            xhr.abort();
