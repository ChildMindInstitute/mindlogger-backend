...
describe('Test widgets that are not covered elsewhere', function () {
    ...
    it('test task progress widget', function () {
        ...
        runs(function () {
            ...
            girderformindlogger.utilities.eventStream.on('g:error', onErrorEvent);
            onProgressEvent = jasmine.createSpy('onErrorEvent');
            girderformindlogger.utilities.eventStream.on('g:event.progress', onProgressEvent);
        ...
        runs(function () {
            girderformindlogger.utilities.eventStream.close();
        ...
        runs(function () {
            girderformindlogger.utilities.eventStream.open();
            girderformindlogger.rest.restRequest({
            ...
            girderformindlogger.utilities.eventStream.off('g:error', onErrorEvent);
            girderformindlogger.utilities.eventStream.off('g:event.progress', onProgressEvent);
        }, 'clean up testing changes');
    });
});

describe('Test folder info widget async fetch', function () {
    var folders = new girderformindlogger.collections.FolderCollection();
    ...
    it('fetch the current user\'s folders', function () {
        runs(function () {
            ...
            folders.fetch({
                parentType: 'user',
                parentId: girderformindlogger.auth.getCurrentUser().id
            });
        });
    ...
    it('show a folder info widget for one of the folders', function () {
        ...
        var widget = new girderformindlogger.views.widgets.FolderInfoWidget({
