...
describe('Test the model class', function () {
    ...
    beforeEach(function () {
        ...
        spyOn(girderformindlogger.rest, 'restRequest').andCallFake(function (opts) {
            var resp = $.Deferred();
            if (triggerRestError) {
                resp.reject('err');
            } else {
    ...
    it('test the base model', function () {
        var SampleModel = girderformindlogger.models.Model.extend({
        ...
        expect(girderformindlogger.rest.restRequest.callCount).toBe(0);
        model.resourceName = 'sampleResource';
        model.save();
        expect(girderformindlogger.rest.restRequest.callCount).toBe(1);
        expect(girderformindlogger.rest.restRequest).toHaveBeenCalledWith({
            url: 'sampleResource',
            method: 'POST',
            data: {
                count: 12,
        ...
        // Test save to update
        girderformindlogger.rest.restRequest.reset();
        ...
        model.save();
        expect(girderformindlogger.rest.restRequest.callCount).toBe(2);
        ...
        // test downloadUrl
        expect(model.downloadUrl()).toBe(girderformindlogger.rest.getApiRoot() + '/sampleResource/' + id + '/download');
        expect(model.downloadUrl({ foo: 'bar' })).toBe(girderformindlogger.rest.getApiRoot() + '/sampleResource/' + id + '/download?foo=bar');
