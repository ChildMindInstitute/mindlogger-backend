...
describe('Test routing paths', function () {
    ...
    it('collect ids for tests', function () {
        _getFirstId(girderformindlogger.collections.UserCollection, ids, 'admin');
        _getFirstId(girderformindlogger.collections.CollectionCollection, ids, 'collection');
        _getFirstId(girderformindlogger.collections.FolderCollection, ids,
            'collectionFolder',
            function () {
                return { parentId: ids.collection, parentType: 'collection' };
            });
        _getFirstId(girderformindlogger.collections.FolderCollection, ids, 'userFolder',
            function () {
                return { parentId: ids.admin, parentType: 'user' };
            });
        _getFirstId(girderformindlogger.collections.GroupCollection, ids, 'group');
        _getFirstId(girderformindlogger.collections.AssetstoreCollection, ids, 'assetstore');
        _getFirstId(girderformindlogger.collections.ItemCollection, ids, 'item',
            function () {
                return { folderId: ids.userFolder };
            });
        _getFirstId(girderformindlogger.collections.ItemCollection, ids, 'file',
            function (coll) {
    ...
    it('check parseQueryString', function () {
        runs(function () {
            ...
            for (var i = 0; i < testVals.length; i += 1) {
                ...
                expect($.param(girderformindlogger.misc.parseQueryString(encode))).toBe(encode);
            }
        });
    });
});
...
describe('Test disabling the router at runtime', function () {
    ...
    it('router should be enabled by default', function () {
        girderformindlogger.router.navigate('collections', { trigger: true });

        expect(router.navigate).toHaveBeenCalled();
    });

    it('disabling router should make navigate() a no-op', function () {
        girderformindlogger.router.enabled(false);

        girderformindlogger.router.navigate('users', { trigger: true });
