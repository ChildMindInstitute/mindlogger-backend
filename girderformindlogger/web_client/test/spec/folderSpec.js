...
function _editFolder(button, buttonText, testValidation) {
    ...
    if (testValidation) {
        ...
        runs(function () {
            $('#g-name').val(oldval);
            girderTest.sendFile('girderformindlogger/web_client/test/testFile.txt',
        ...
        runs(function () {
            girderTest.sendFile('girderformindlogger/web_client/test/fake.jpg',
                '.g-markdown-drop-zone .g-file-input');
        });

        waitsFor(function () {
            return $('#g-dialog-container .g-markdown-text').val().indexOf(
                '![fake.jpg](' + girderformindlogger.rest.getApiRoot()) !== -1;
