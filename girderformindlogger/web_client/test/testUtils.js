...
girderTest.createUser = function (login, email, firstName, lastName, password, userList) {
    return function () {
        ...
        runs(function () {
            ...
            if (userList) {
                userList.push(girderformindlogger.auth.getCurrentUser());
            }
        });
    };
};
...
girderTest.logout = function (desc) {
    return function () {
        ...
        waitsFor(function () {
            return girderformindlogger.auth.getCurrentUser() === null;
        }, 'user to be cleared');
...
girderTest.waitForLoad = function (desc) {
    ...
    var clearCount = 0;
    waitsFor(function () {
        if (girderformindlogger._inTransition ||
                girderformindlogger.rest.numberOutstandingRestRequests() ||
...
girderTest.waitForDialog = function (desc) {
    ...
    waitsFor(function () {
        return !girderformindlogger._inTransition;
...
girderTest.binaryUpload = function (path) {
    ...
    runs(function () {
        ...
        girderformindlogger.router.navigate(old, { trigger: true });
    });
...
girderTest.testRoute = function (route, hasDialog, testFunc) {
    runs(function () {
        ...
        girderformindlogger.router.navigate(route, { trigger: true });
    });
 ...
 *
 * To use, start girderformindlogger in testing mode: `girderformindlogger serve --mode=testing` and
    ...
    girderTest.promise = girderTest.promise
        .then(function () {
            ...
            $.fn.emulateTransitionEnd = function (duration) {
                girderformindlogger._inTransition = true;
                var called = false;
                var $el = this;
                ...
                var callback = function () {
                    ...
                    girderformindlogger._inTransition = false;
                };
            ...
            girderformindlogger.app.events = girderformindlogger.app.events || {};
            girderformindlogger.app.events['click a'] = function (evt) {
                if (!evt.isDefaultPrevented()) {
                    var elem = $(evt.target),
                        href = elem.attr('href');
                    if (elem.is('a') && href && href.substr(0, 1) === '#') {
                        girderformindlogger.router.navigate(href.substr(1), { trigger: true });
                        evt.preventDefault();
                    }
                }
            };
            girderformindlogger.app.delegateEvents();
            return girderformindlogger.app.start();
        })
        .then(function () {
            girderformindlogger.events.trigger('g:appload.after', girderformindlogger.app);
            return girderformindlogger.app;
        });
